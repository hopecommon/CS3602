...
def get_prerope_query_states(module: nn.Module, hidden_states: torch.Tensor) -> torch.Tensor:
    ...
    elif isinstance(module, GPTNeoXAttention) or hasattr(module, "query_key_value"):
        num_heads = module.config.num_attention_heads
        head_dim = module.head_size if hasattr(module, "head_size") else module.head_dim
...
def get_prerope_key_states(module: nn.Module, hidden_states: torch.Tensor) -> torch.Tensor:
    ...
    if isinstance(module, Phi3Attention):
        ...
        head_dim = module.head_size if hasattr(module, "head_size") else module.head_dim
        qkv = module.query_key_value(hidden_states)
        qkv = qkv.view(bsz, k_len, num_heads, 3 * head_dim).transpose(1, 2)
